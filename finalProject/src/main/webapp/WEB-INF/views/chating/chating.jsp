   ...
   ws.onopen = function() {
	//처음 접속 시에만 채팅방에 추가함(현재 방정보도 같이 출력)	   
      	...
      	$("#buttonMessage").click(function() {
         	let msg = $('input[name=chatInput]').val().trim("!%/"); //메시지
         
         	let wisper = $("#wisper").val().trim("!%/"); //귓속말 대상자
      ...
      //엔터키 입력처리
      	...
      	$("#searchRoomBtn").click(function() {
      		...
      		//검색어
      		let text = $("#searchRoomText").val().trim("!%/&");
      		ws.send("!%/&"+text);
  ...
  //서버로 부터 받은 메세지 보내주기
   	ws.onmessage = function(message) {
    	...
    	//메세지 
      	...
      	//접속자리스트
      	let jsonData2 = JSON.parse(message.data);
      	if(jsonData2.list !=null){
         	$("#listPeople").html(jsonData2.list);
      	}
      	
      	//방 정보
      	let jsonData3 = JSON.parse(message.data);
      	if(jsonData3.room !=null){
      		//alert(jsonData3.room);
			let roomSplit = jsonData3.room.split(',');
      		
			let str = "<div class='item'><div class='content'>"+
